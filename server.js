        ...
        try {
            ...
            if (
                ...
                return res.status(400).json({
                    success: false,
                    message:
                        "Nama game dan App ID wajib diisi."
                });
            }
        ...
        try {
            ...
            if (
                ...
                return res.status(409).json({
                    success: false,
                    message:
                        "Masih ada idle session yang berjalan."
                });
            }
    ...
    async (req, res) => {
        try {
            ...
            res.status(500).json({
                success: false,
                message:
                    "Gagal menghentikan idle session."
            });
        }
    }
);
...
let internalWorkerRunning = false;

async function runInternalWorker() {
    if (internalWorkerRunning) {
        return;
    }

    internalWorkerRunning = true;
...
app.listen(
    ...
    () => {
        ...
        console.log(
            "Internal Worker: ENABLED"
        );

        console.log(
            "Worker Mode: STATUS ONLY"
        );
